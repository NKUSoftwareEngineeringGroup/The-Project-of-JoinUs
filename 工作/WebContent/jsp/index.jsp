          ...
          <table width="720" border="1" style="border-style:none;">
            <tr>
             ...
             <% if(act.getUccid().equals("小诚")){
               ...
               <%} else{%>
                   <div><a href="/Joinus/ShowInxiaocheng"><img src="/Joinus/upload/images/photo/<%=act.getUphoto()%>" width="50" height="50"  alt=""/></a><span><%=act.getUname()%>于<%=act.getPdate()%>转发了由<%=act.getOwner_id()%>发布的：</span></div>
             <%}}
             else{ 
             if(act.getPtype()==1){ %>
               <div><a href="/Joinus/ShowInOthersServlet?uccid=<%=act.getUccid()%>"><img src="/Joinus/upload/images/photo/<%=act.getUphoto()%>" width="50" height="50"  alt=""/></a><span><%=act.getUname()%>于<%=act.getPdate()%>参加了由<%=act.getOwner_id()%>发布的：</span></div>
             <%}else if(act.getPtype()==0){ %>
                <div><a href="/Joinus/ShowInOthersServlet?uccid=<%=act.getUccid()%>"><img src="/Joinus/upload/images/photo/<%=act.getUphoto()%>" width="50" height="50"  alt=""/></a><span><%=act.getUname()%>于<%=act.getPdate()%>发布了：</span></div>
             <%} else{%>
                 <div><a href="/Joinus/ShowInOthersServlet?uccid=<%=act.getUccid()%>"><img src="/Joinus/upload/images/photo/<%=act.getUphoto()%>" width="50" height="50"  alt=""/></a><span><%=act.getUname()%>于<%=act.getPdate()%>转发了由<%=act.getOwner_id()%>发布的：</span></div>
             <%} }%>
         <div>
           <table width="603" border="0" align="right">
             <tr>
               <td width="550"><a href="/Joinus/ToCommentServlet?aid=<%=act.getAid()%>">活动名称：<%=act.getAname()%></a></td>
             </tr>
             <tr>
